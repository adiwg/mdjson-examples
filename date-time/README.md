...
A minimally valid mdEditor record used to test the import of date-time values. Created using mdEditor version 1.3.1 (mdJSON schema v 2.10.2). The recordset contains seven metadata records labeled:
...
The remaining records each specify a "TimePeriod StartDate" precision as indicated by the name (e.g., "YM Time Period" records the date as `"startDateTime":"2024-05"`).

### dateTest-mdjson-20260204.json

The "Citation date test" metadata record from the above "dateTest-mdeditor-20260204.json" recordset, exported as mdJSON using mdEditor version 1.3.1 (mdJSON schema v 2.10.2).
